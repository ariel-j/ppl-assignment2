...
const parseLetExp = (bindings: Sexp, body: Sexp[]): Result<LetExp> => {
    ...
    const vars = map(b => b[0], bindings); //((a cexp) (b 4) (c 5)) => (a b c)
...
export const parseClassExp = (params: Sexp[]): Result<ClassExp> => 
    params.length !== 2 ? makeFailure(`Expression not of the form ( class ( <var>+ <binding>+)): ${format(params)}`) :
    isArray(params[0]) &&  isGoodBindings(params[1]) ? 
        // Returns a result with classexp 
        mapv(
            // Parse the vals in the bindings array                                                                         
            mapv(mapResult(parseL3CExp, map(second, params[1] as [string, Sexp][])),
                // Make a binding variable
                (vals: CExp[]) : Binding[] => zipWith(makeBinding, map(b => b[0], params[1] as [string, Sexp][]), vals)),
                // Make classExp for the bindings and varDecl
                (bindings : Binding[]) : ClassExp => makeClassExp(map(makeVarDecl, params[0] as string[]) , bindings))
        : makeFailure(`Invalid vars or bindings: \nvars: ${format(params[0])} \nbindings: ${format(params[1])}`);

        
    /*return mapv(mapv(mapResult(parseL3CExp, map(second, params[1])),
                (vals: CExp[]) => zipWith(makeBinding, map(b => b[0], params[1]), vals)),
        (bindings: Binding[]) => makeClassExp(map(makeVarDecl, params[0]), bindings))
    a. map(second, params[1]): extracts the expressions (values) from the method bindings.
    b. mapResult(parseL3CExp, map(second, params[1])): parses each method expression into a CExp.
    c. mapv(mapResult(parseL3CExp, map(second, params[1])),(vals: CExp[]) => zipWith(makeBinding, map(b => b[0], params[1]), vals)):
    If parsing succeeds, it takes the parsed expressions and combines them with the method names
    d. zipWith(makeBinding, map(b => b[0], params[1]), vals)): creates Binding objects for each method, pairing method names with their parsed expressions
    e. mapv(...): the outer mapv then takes these bindings and creates a ClassExp: It also converts the field names into VarDecl objects.
...
const unparseClassExp = (exp: ClassExp): string => {
    ...
    const methods = exp.methods.map(method => 
        `(${method.var.var} ${unparseL3(method.val)})`
    ).join(" ");
